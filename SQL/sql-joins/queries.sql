...
SELECT first_name, last_name, COUNT(owner_id) FROM owners 
    JOIN vehicles ON owners.id=owner_id GROUP BY (first_name, last_name) ORDER BY first_name;
...
JOIN vehicles on owners.id=owner_id GROUP BY (first_name, last_name) 
